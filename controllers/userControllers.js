    ...
    try {
        ...
        if (!data) {
            ...
            if (!schema.validate(password)) {
                return res.status(400).json({
                    success: false,
                    message: "Password not match for schema"
                })
            }
            const hashedPassword = await bcrypt.hash(password, saltRounds);
...
const changePassword = async (req, res) => {
    try {
        ...
        const compairPassword = await bcrypt.compare(currentPassword, data.password)
        if (!compairPassword) {
            return res.status(404).json({
                success: false,
        ...
        if (!schema.validate(newPassword)) {
            return res.status(400).json({
                success: false,
                message: "New password does not meet schema requirements"
            })
        }
